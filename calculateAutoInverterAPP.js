const Inverter = require('./infrastructure/inverter/inverter');
...
exports.lambdaHandler = async (event) => {
  ...
  const allResult = allInverters.map(inverter => {
    const possiblePlans = Wiring.calculateWiringRestriction(
      ...
      Number(inverter.paco), Number(inverter.mpptLow), Number(inverter.mpptLow),
    ...
    return Wiring.calculateWiring(PVParams, totalPanels, possiblePlans);
  });
